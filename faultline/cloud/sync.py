...
def pull_feature_map(repo_slug: str) -> dict[str, Any] | None:
    """Fetch the latest scan for ``repo_slug`` with server-side overrides
    (rename/aliases/labels) already applied.

    Returns the merged feature_map dict, or None on no-op / failure.
    """
    key = _api_key()
    if not key:
        return None
    ...
    try:
        ...
        logger.warning("httpx not installed — cannot pull from cloud")
        return None

    try:
        response = httpx.get(
            f"{_api_base()}/scans/latest",
        ...
        if response.status_code >= 400:
            logger.warning(
                "cloud pull failed: %d %s", response.status_code, response.text[:200],
            )
            return None
        return response.json()
    except Exception as exc:
        logger.warning("cloud pull error: %s", exc)
        return None
